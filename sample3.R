...
sort_grid_capture <- function(grid, metric_type) {
  ...
  list_models <- lapply(grid@model_ids, function(model_id) h2o.getModel(model_id))
  
  # initialize temporary vectors to store values to construct data frame
  vec_model_id <- vector()
  vec_capture_rate <- vector()
  vec_data_fraction <- vector()
  vec_total_response_rate <- vector()
  
  # loop through each model and grab the 1% capture rate
  num_models <- length(list_models)
  for (i in 1:num_models) {
    cur_model <- list_models[[i]]
    
    vec_model_id <- c(vec_model_id, cur_model@model_id)
    
    # get the appropriate gains/lift table
    if (metric_type == "training") {
      df_gains_lift_table <- cur_model@model$training_metrics@metrics$gains_lift_table
    } else if (metric_type == "validation") {
      df_gains_lift_table <- cur_model@model$validation_metrics@metrics$gains_lift_table
    } else {
      df_gains_lift_table <- cur_model@model$cross_validation_metrics@metrics$gains_lift_table
    }
    
    num_rows <- nrow(df_gains_lift_table)
    
    # first capture rate is the 1% capture rate
    cur_capture_rate <- df_gains_lift_table$capture_rate[[1]]
    cur_data_fraction <- df_gains_lift_table$cumulative_data_fraction[[1]]
    cur_total_response_rate <- df_gains_lift_table$cumulative_response_rate[[num_rows]]
    
    vec_capture_rate <- c(vec_capture_rate, cur_capture_rate)
    vec_data_fraction <- c(vec_data_fraction, cur_data_fraction)
    vec_total_response_rate <- c(vec_total_response_rate, cur_total_response_rate)
  }
  
  df_capture_rate <- data.frame(vec_model_id, vec_capture_rate, vec_data_fraction, vec_total_response_rate)
  colnames(df_capture_rate) <- c("model_id", "capture_rate", "data_fraction", "total_response_rate")
  df_capture_rate_sorted <- df_capture_rate[order(df_capture_rate$capture_rate, decreasing=TRUE),]
  return(df_capture_rate_sorted)
